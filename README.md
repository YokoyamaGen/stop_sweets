...
### サービスを後から利用開始したユーザに対してもお菓子を止めるモチベーション維持に配慮した点です。
...
お菓子を食べてしまった日を月初にクリアするための実装です。

先月にユーザがお菓子を食べてしまった日数を月初にクリアするためにバッチ処理で実装するようにいたしました。バッチ処理を実装するにあたり、Heroku上ではgem「whenever」が使用できなかったため、Herokuのスケジューラを使用することを検討しました。Herokuのスケジューラで、設定可能なバッチ処理の起動タイミングは毎日１回で、月１回はございませんでした。そのため、**どうやったら実装できるかを諦めずに考えました。**　
